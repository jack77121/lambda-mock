...
class RunSimulationRequest(BaseModel):
    ...
    evaluate_var_result_id: str = Field(
        ..., description="評估變數結果ID (用於群組識別)"
    )
    mode_key: str = Field(
        ..., description="模式組合鍵 (例如: '電價套利-日選2h-即時單一')"
    )
    task_id: str = Field(..., description="此次模擬的唯一識別碼")


class RunSimulationResultData(BaseModel):
    task_id: str = Field(..., description="模擬ID")
    evaluate_var_result_id: str = Field(..., description="群組ID")
    task_result: Optional[dict] = Field(None, description="計算結果")
...
class RunSimulationResponse(BaseModel):
    ...
    result: RunSimulationResultData = Field(..., description="計算結果")
