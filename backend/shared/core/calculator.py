...
def calculate_transferable_energy(
    ...
    mask = (df_ami['timestamp'] >= time_periods[0][0]) & (df_ami['timestamp'] <= time_periods[0][1])
    df_segment = df_ami.loc[mask].copy()

    df_segment['可放電功率'] = np.where(df_segment['tou_level'] == 'high', df_segment['load_kw'].clip(upper=pcs_max_kW), 0)
    df_segment['可充電功率'] = np.where(
        df_segment['tou_level'] == 'low',
        np.minimum(contract_capacity - df_segment['load_kw'], pcs_max_kW).clip(lower=0),
        0
    )
    df_ami2 = df_segment.groupby(['season', 'weekday2']).agg({'可放電功率': 'sum', '可充電功率': 'sum'}).div(4).reset_index().rename(columns={'可放電功率': '可放電量', '可充電功率': '可充電量'}).round(2)

    # 若考慮 large consumer，直接統計 18:00~19:45 這段時段
    if consider_large_consumer:
        mask_lc = (df_segment['timestamp'] >= "18:00") & (df_segment['timestamp'] <= "19:45")
        df_lc = df_segment.loc[mask_lc].copy()
        lc_stats = df_lc.groupby(['season', 'weekday2'])['可放電功率'].sum().div(4).reset_index().rename(columns={'可放電功率': '用電大戶義務可放電量'}).round(2)
        df_ami2 = pd.merge(df_ami2, lc_stats, on=['season', 'weekday2'], how='left')
    return df_segment, df_ami2


def batch_calculate_transfered_energy(df_ami2, avail_kWh_series, season, rtt_loss_rate, consider_large_consumer=False):
    """
    ...
    for avail_kWh in avail_kWh_series:
        df_season['可轉移電量'] = np.minimum(
            avail_kWh*np.sqrt(1-rtt_loss_rate), np.minimum(df_season['可放電量'], df_season['可充電量'])
        )
        mean_kWh = df_season[df_season['weekday2'].isin(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'])]['可轉移電量'].mean()
    ...
    if consider_large_consumer and '用電大戶義務可放電量' in df_season.columns:
        for avail_kWh in avail_kWh_series:
            df_season['用電大戶義務可轉移電量'] = np.minimum(
                avail_kWh*np.sqrt(1-rtt_loss_rate), np.minimum(df_season['用電大戶義務可放電量'], df_season['可充電量'])
            )
            mean_kWh = df_season[df_season['weekday2'].isin(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'])]['用電大戶義務可轉移電量'].mean()
...
# 篩出固定時間區間的資料
def filter_summer_week_data(df, start_time_str, end_time_str, only_summer=True, only_weekday=True):
    """
    ...
    mask = (df_filtered['timestamp'].dt.time >= start_time) & (df_filtered['timestamp'].dt.time < end_time)
    return df_filtered[mask]

# 計算 最小的DR容量
def calculate_dr_capacity(df_ami, start_time, end_time, dr_hr, bms_kWh):
    # 計算抑低時段長度（小時）
    dr_avail_kWh = bms_kWh / dr_hr

    # 夏月工作日：計算平均可放電量
    df_filtered2 = filter_summer_week_data(df_ami, start_time, end_time, only_summer=True, only_weekday=True)
    df_grouped = df_filtered2.groupby('weekday2').agg({
        '可放電功率': 'mean',
        'weekday': 'count'
    }).reset_index()
    ...
    # 考慮放電限制
    df_grouped['平均最大可放電量'] = np.minimum(df_grouped['平均可放電量'], dr_avail_kWh)

    # 非夏月工作日 baseline
    df_baseline = filter_summer_week_data(df_ami, start_time, end_time, only_summer=False, only_weekday=True)
    df_baseline = df_baseline.groupby('weekday2').agg({
        '可放電功率': 'mean',
        'weekday': 'count'
    }).reset_index()

    # 最大可抑低量：取 baseline 和最大可放電量的最小值
    df_grouped['最大可抑低量'] = np.minimum(df_grouped['平均最大可放電量'], df_baseline['可放電功率'])
...
def compute_spinning_summary(df_ami_week, max_bms_kW, avail_kWh):
    ...
    # 計算小時數與平均可放電功率
    df_ami_week2 = df_ami_week.groupby(['season', 'spinning_level']).agg({
        '可放電功率': 'mean',
        'tou': 'count',
        'load_kw': 'mean'
    }).reset_index().rename(columns={'tou': '小時數'})

    df_ami_week2['小時數'] = df_ami_week2['小時數'] / (4 * 5)
    df_ami_week2['總放電時數'] = avail_kWh / df_ami_week2['可放電功率']

    # 計算負載統計
    df_stats = df_ami_week.groupby(['season', 'spinning_level'], group_keys=False).apply(
...
def compute_spinning_gain_A(series_ami_weekend, non_working_days, capacity_price, performance_price):
    """
    計算非工作日的即時備轉收益
    """
    ...
    # 單一用戶收益計算
    sp_A_case_single = non_working_days * 24 * rate * series_ami_weekend['load_kw_gt1000_mean'] / 1000 * series_ami_weekend['gt1000_ratio']

    # 聚合後收益計算
    sp_A_case_agg = non_working_days * 24 * rate * series_ami_weekend['load_kw_mean'] / 1000

    return sp_A_case_single, sp_A_case_agg

# B. 計算工作日的即時備轉收益，包括夏月跟非夏月
# 同時考慮執行需量反應負載管理措施的情境
def compute_spinning_gain_B(df, season, days, capacity_price, performance_price, dr_hour, cycles):
    # 尖峰的行為
    ...
    daily_benefit_single = (
        (capacity_price + performance_price) * (小時數_1 - 不可投標時數_1) * row_1['load_kw_gt1000_mean'] / 1000 * row_1['gt1000_ratio'] +
        (capacity_price + performance_price) * (小時數_0 - 不可投標時數_0) * row_0['load_kw_gt1000_mean'] / 1000 * row_0['gt1000_ratio']
    )
    total_single = days * daily_benefit_single

    # 聚合場域計算
    daily_benefit_agg = (
        (capacity_price + performance_price) * (小時數_1 - 不可投標時數_1) * row_1['load_kw_mean'] / 1000 +
        (capacity_price + performance_price) * (小時數_0 - 不可投標時數_0) * row_0['load_kw_mean'] / 1000
    )
...
def compute_total_spinning_gain_sum(
    ...
    # Case A: 非工作日收益
    sp_A_single, sp_A_agg = compute_spinning_gain_A(series_ami_weekend,non_working_days, capacity_price, performance_price)
    # print('case A')
    # print('sp_A_single', sp_A_single)
    # print('sp_A_agg', sp_A_agg)

    # Case B: 非夏月收益
    sp_B_single_ns, sp_B_agg_ns = compute_spinning_gain_B(df_ami_week2, 'not_summer', working_days_not_summer, capacity_price, performance_price, dr_hour, cycles)

    # Case B: 夏月收益
    sp_B_single_s, sp_B_agg_s = compute_spinning_gain_B(df_ami_week2, 'summer', working_days_summer, capacity_price, performance_price, dr_hour, cycles)
...
def calculate_max_arbitrage_profit(config):
    ...
    # 理論收益計算
    a = (夏月最高電價 - 夏月最低電價) * 夏月天數 * 夏月每日可轉移度數 + (非夏月最高電價 - 非夏月最低電價) * 非夏月天數 * 非夏月每日可轉移度數 

    b = 夏月最低電價 * 夏月天數 * 夏月每日可轉移度數 * 損失率 + \
...
def calculate_max_large_consumer_profit(config, lc_mode):
    ...
    # 最大收益計算
    max_profit = compute_large_consumer_reduction(
        義務裝置容量, 總放電度數, lc_mode
    )
    if max_profit == 0:
        return 1.0
...
def calculate_max_spinning_service_profit(config):
    ...
    非夏月工作日執行時數 = (
        24 - max(日選執行時數, 電池放電時數) * 2 - 電池放電時數 * 2 * (每日最大循環次數-1)
    )
    非工作日執行時數 = 24
...
def get_arbitrage_formula_display_data(config, score = None):
    ...
    return {
        ...
        "參數": [
            {"label": "儲能容量", "value": config["儲能系統"]["儲能容量"], "unit": "kWh"},
            {"label": "SOC上限", "value": config["儲能系統"]["SOC上限"], "unit": "%"},
            {"label": "SOC下限", "value": config["儲能系統"]["SOC下限"], "unit": "%"},
            {"label": "每日最大循環次數", "value": config["儲能系統"]["每日最大循環次數"], "unit": "次"},
            {"label": "損失率", "value": config["儲能系統"]["電能損失率(Round Trip)"], "unit": "%"},
            {"label": "夏月天數", "value": config["電價方案"]["夏月天數"], "unit": "天"},
            {"label": "非夏月天數", "value": config["電價方案"]["非夏月天數"], "unit": "天"},
            {"label": "夏月最高電價", "value": config["電價方案"]["調整後夏月最高電價"], "unit": "元/kWh"},
            {"label": "夏月最低電價", "value": config["電價方案"]["調整後夏月最低電價"], "unit": "元/kWh"},
            {"label": "非夏月最高電價", "value": config["電價方案"]["調整後非夏月最高電價"], "unit": "元/kWh"},
            {"label": "非夏月最低電價", "value": config["電價方案"]["調整後非夏月最低電價"], "unit": "元/kWh"}
        ],
...
def get_large_consumer_formula_display_data(config, score=None):
    ...
    return {
      ...
      "參數": [
            ...
            {"label": "SOC下限", "value": config["儲能系統"]["SOC下限"], "unit": "%"},
            {"label": "損失率", "value": config["儲能系統"]["電能損失率(Round Trip)"], "unit": "%"},
            {"label": "PCS標稱功率", "value": config["儲能系統"]["PCS 標稱功率"], "unit": "kW"},
            {"label": "義務裝置容量", "value": config["再生能源義務用戶"]["義務裝置容量"], "unit": "kW"},
            {"label": "夏月天數", "value": config["電價方案"]["夏月天數"], "unit": "天"},
            {"label": "非夏月天數", "value": config["電價方案"]["非夏月天數"], "unit": "天"}
        ],
        "策略建議": get_strategy_suggestion(score)
    ...
    def get_strategy_suggestion(score):
        if score is None or score == 0:
            ...
            return {
                ...
                "suggestions": [
                    "離峰時段無法充分充電，把電池充飽。",
                    "日選時段的下午6-8時(2H)、下午4-8時(4H)、下午4-10時用電不足，導致未能完全放電，顯示電池配置容量可能太大！",
                ]
    }
    return {
        "公式解釋": [
        ...
        "參數": [
            ...
            {"label": "損失率", "value": config["儲能系統"]["電能損失率(Round Trip)"], "unit": "%"},
            {"label": "PCS標稱功率", "value": config["儲能系統"]["PCS 標稱功率"], "unit": "kW"},
            {"label": "執行時數", "value": config["日選時段型"]["執行時數"], "unit": "小時"},
            {"label": "當日執行率", "value": config["日選時段型"]["當日執行率"], "unit": "%"},
            {"label": "扣減電價", "value": config["日選時段型"]["流動電費扣減費率"], "unit": "元/kWh"},
            {"label": "扣減倍率", "value": config["日選時段型"]["扣減比率"], "unit": "%"},
            {"label": "可參與天數", "value": config["日選時段型"]["5月-10月可參與天數"], "unit": "天"}
        ],
         "策略建議": get_strategy_suggestion(score)       
    }
